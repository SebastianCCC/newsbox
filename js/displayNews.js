window.addEventListener('DOMContentLoaded', () => {
  axios.get('js/json/newsData.json').then((response) => {
    ...
    let cardStorage = localStorage['archive'] ? JSON.parse(localStorage['archive']) : []

    const idArray = []
    cardStorage.forEach((article) => idArray.push(article.id))
    console.log(idArray)
      ...
      axios
        ...
        .then((response) => {
          ...
          for (let i = 0; i < newspath.length; i++) {
            if (
              !idArray.includes(newspath[i].short_url.replace('https://nyti.ms/', 'id_')) &&
              newspath[i].section != 'admin' &&
              newspath[i].item_type != 'Promo' &&
              newspath[i].abstract != ''
            ) {
